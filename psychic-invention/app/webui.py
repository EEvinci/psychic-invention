...
with gr.Blocks() as block:
    with gr.Column():
        gr.HTML('<h1>生成高成效图片的Prompt生成器</h1>')
        with gr.Tab('Prompt生成'):
            input = gr.Textbox(lines=6, label='你的大致描述', placeholder='在此输入内容...')
            output = gr.Textbox(lines=6, label='优化后的Prompt')
            submit_btn = gr.Button('让我想想')
